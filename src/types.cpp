...
void initTexture(Texture* self, SDL_Texture* origin, int width, int height,
  ...
  self->height = height;
  self->frames = frames;
  self->crops = (SDL_Rect*)malloc(sizeof(SDL_Rect) * frames);
...
void initEffect(Effect* self, int duration, int length, SDL_BlendMode mode) {
  self->keys = (SDL_Color*)malloc(sizeof(SDL_Color) * length);
...
void copyEffect(const Effect* src, Effect* dest) {
  ...
  dest->keys = (SDL_Color*)malloc(sizeof(SDL_Color) * src->length);
...
LinkNode* createLinkNode(void* element) {
  LinkNode* self = (LinkNode*)malloc(sizeof(LinkNode));
  initLinkNode(self);
